                    ...
                    <div className='product-detail-wrapper'>
                        ...
                        <div className='product-detail-items'>
                            <div style={{ paddingLeft: "50px" }} >
                                <p className='title is-1' style={{ margin: 0, paddingBottom: "5px" }}>{productData.name}</p>
                                <p className='has-text-weight-light is-6' style={{ margin: 0, paddingBottom: "5px" }}>{productData.description}</p>
                                <p className='subtitle is-4' style={{ margin: 0, paddingBottom: "5px" }}>$ {formatCurrency(productData?.price)}</p>
                                <div>
                                    {componentData && componentData.map((component, index) => {
                                        return (
                                            <>
                                                <p className='has-text-weight-light is-6' style={{ margin: 0, paddingBottom: "5px" }}>Các loại {component.name}</p>
                                                <div style={{ display: "flex" }}>
                                                    {component?.components && component?.components.map((img, index) => {
                                                        return (
                                                            <div className='product-component-style-items'>
                                                                <figure className="image is-32x32" style={{ overflowY: "hidden", borderRadius: "10px" }}>
                                                                    <img src={img?.image} alt='' style={{ objectFit: 'cover' }}></img>
                                                                </figure>
                                                            </div>
                                                        )
                                                    }
