...
const CustomerBodyProfile = () => {
  ...
  const { data: getBodyProfile, Loading } = useQuery("get-all-bodyProfile", () =>
          ...
          <>
            <tr key={profile.id}>
              ...
              <td style={{ display: "flex" }}>
                ...
                <div style={{ paddingLeft: 10, paddingRight: 10, cursor: 'pointer', overflow: "hidden", textAlign: "center" }}>
...
const CreateBodyProfile = ({ isModalOpen, getAllBodyAttributes, isLoading, handleCancel }) => {
  ...
  const handleCreateBodySize = async () => {
    console.log("DATA:", typeof (inputValues))
    console.log("NAME:", bodyProfileName)
  ...
  const { data: getBodyAttributesDetail, Loading } = useQuery("get-all-bodyAtrributes", () =>
    fetch(`https://etailorapi.azurewebsites.net/api/profile-body/`, {
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
